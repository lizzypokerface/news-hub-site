...
| **United States** | `1.8%` (Slowing growth). Fiscal stance is tightening amid political gridlock over spending. | `2.5%` CPI / `4.1%` Unemp. Persistent cost-of-living pressures despite moderating inflation. | `-3.4%` (Structural Debtor Nation). Relies on the dollar's reserve status to attract foreign capital to fund deficits. | `127%` (Very high). Manageable due to sovereign currency, but a source of intense political conflict and leverage. | `3.5%` (2024 data). Exceptionally high; reflects global military posture and power projection. | Rate: `4.5%` / FX Reserves: `$650B` . Fed policy sets global rates. Reserves are large but secondary to dollar hegemony. |
| **Japan** | `0.9%` (Anemic growth). Fiscal stance remains supportive, trying to entrench inflation. | `2.2%` CPI / `2.5%` Unemp. Wage growth is key as nation shifts from decades of deflation. | `+3.2%` (Major Creditor Nation). A primary global capital exporter, funding deficits elsewhere (e.g., the US). | `258%` (Extremely high). Not a crisis as it's domestically held and monetized by the central bank. | `1.2%` (2024 data). Historically low but rising quickly in response to regional tensions. | Rate: `0.5%` / FX Reserves: `$1.2T` . Policy is exiting an era of extreme stimulus. Large reserves provide a buffer. |
| **Germany** | `1.2%` (Weak recovery). Fiscal stance is constrained by constitutional "debt brake, " limiting stimulus. | `2.4%` CPI / `5.9%` Unemp. Risk of deindustrialization due to high energy costs is the main concern. | `+6.8%` (Massive Creditor Nation). Export-led model creates huge surpluses and suppresses domestic demand. | `64%` (Low for G7). Hawkish fiscal policy prioritizes debt reduction over investment, a core tenet of ordoliberalism. | `2.1%` (2024 data). Rapidly increasing post-2022, but from a low base, reflecting a major policy shift. | Rate: `3.0%` (ECB). Monetary policy is not sovereign. Fiscal policy is constrained by EU/domestic rules. |
| **United Kingdom** | `1.4%` (Stagnant). Post-Brexit adjustments and fiscal austerity continue to drag on growth. | `2.3%` CPI / `4.4%` Unemp. A prolonged "cost of living crisis" with real wages struggling to recover. | `-2.5%` (Persistent Debtor Nation). Reliant on "the kindness of strangers" to fund its external deficit. | `104%` (High and rising). Significant pressure for fiscal consolidation (austerity) regardless of party in power. | `2.3%` (2024 data). Consistently above NATO target, reflecting legacy power ambitions. | Rate: `4.25%` / FX Reserves: `$180B` . Policy often caught between fighting inflation and preventing recession. |
| **France** | `1.3%` (Sluggish). Fiscal stance is one of consolidation after passing controversial pension reforms. | `2.5%` CPI / `7.2%` Unemp. High structural unemployment and social unrest are persistent features. | `-1.1%` (Minor Debtor Nation). More balanced than many peers but still reliant on external financing. | `111%` (High). Faces pressure from EU fiscal rules to implement further austerity measures. | `2.0%` (2024 data). A significant military power with an independent nuclear deterrent. | Rate: `3.0%` (ECB). Sovereignty is pooled at the EU level; fiscal policy constrained by the Stability and Growth Pact. |
| **Canada** | `1.6%` (Slowing). Housing bubble and household debt are major headwinds. Fiscal stance is neutral. | `2.4%` CPI / `6.2%` Unemp. High cost of housing is the dominant social and economic issue. | `-0.5%` (Near balance). Commodity exports help balance the external account. | `107%` (High). Debt levels across federal and provincial governments are a long-term concern. | `1.3%` (2024 data). Chronically below NATO's 2% target, relies heavily on the US security umbrella. | Rate: `4.0%` / FX Reserves: `$110B` . Monetary policy is heavily influenced by the US Federal Reserve's actions. |
| **Australia** | `1.7%` (Slowing). Growth is highly dependent on commodity exports to China and domestic consumption. | `3.1%` CPI / `4.3%` Unemp. Inflation remains sticky, pressuring household budgets. | `-0.8%` (Minor Debtor Nation). Commodity price cycles dictate the external balance. | `58%` (Low for developed nations). Significant fiscal space, but policy is cautious. | `2.1%` (2024 data). Spending is increasing significantly due to AUKUS pact and regional strategy. | Rate: `4.5%` / FX Reserves: `$95B` . Policy is sensitive to commodity prices and the economic health of China. |
| **South Korea** | `2.2%` (Moderate growth). Highly dependent on semiconductor cycle and exports to US/China. | `2.5%` CPI / `3.0%` Unemp. Low unemployment but high household debt is a major systemic risk. | `+2.8%` (Creditor Nation). Strong surplus based on high-tech exports, but exposed to global supply chains. | `55%` (Low but rising fast). Rapid aging population is creating future fiscal pressures. | `2.8%` (2024 data). High spending reflects its precarious geopolitical position opposite North Korea. | Rate: `3.5%` / FX Reserves: `$420B` . Strong reserves, but policy is highly sensitive to trade and capital flows. |
| **Part B: Global South Economies** |  |  |  |  |  |  |
| **China** | `4.5%` (Slowing but strong). Targeted stimulus to counter property crisis and boost domestic tech. | `1.5%` CPI / `5.5%` Unemp. Deflationary pressures and high youth unemployment are major challenges. | `+1.5%` (Major Creditor Nation). Surplus shrinking but still a key global capital provider via BRI and other channels. | `85%` (Official debt is high). Real figure is much higher if local govt/SOE debt is included; a major systemic risk. | `1.8%` (2024 est.). Second largest military budget globally; focus is on naval expansion and tech parity. | Rate: `3.2%` / FX Reserves: `$3.4T` . High degree of sovereignty via capital controls and world's largest reserve buffer. |
| **India** | `6.5%` (Strong growth). Fiscal stance is focused on infrastructure spending and manufacturing incentives. | `4.8%` CPI / `7.5%` Unemp. High inflation and formal job creation remain persistent challenges. | `-1.5%` (Structural Debtor Nation). Reliant on foreign portfolio investment and remittances to fund its growth. | `82%` (High for EM). Growth helps manage the ratio, but limits fiscal space for social spending. | `2.5%` (2024 data). Third largest spender globally, driven by strategic competition with China and Pakistan. | Rate: `6.0%` / FX Reserves: `$650B` . Moderate reserves provide a buffer, but policy is often reactive to capital flows. |
| **Brazil** | `2.0%` (Moderate growth). Fiscal stance is a battle between social spending goals and market pressure for austerity. | `3.8%` CPI / `8.0%` Unemp. High structural unemployment and inequality are defining features. | `-1.8%` (Commodity-driven Debtor). External balance is highly sensitive to global commodity prices. | `88%` (High for EM). Constant pressure from bond markets for fiscal consolidation, constraining sovereign policy. | `1.1%` (2024 data). Low spending relative to GDP, prioritizing domestic issues over power projection. | Rate: `9.5%` / FX Reserves: `$350B` . Large reserves offer protection, but high rates are needed to attract capital. |
| **South Africa** | `1.2%` (Stagnant). Crippling energy shortages ("loadshedding") and logistics failures constrain all activity. | `5.0%` CPI / `32.5%` Unemp. Extreme unemployment and inequality create a state of permanent social crisis. | `-2.0%` (Debtor Nation). Reliant on foreign capital which is deterred by low growth and instability. | `75%` (High and rising). Debt service costs consume a large portion of revenue, forcing austerity. | `0.9%` (2024 data). Very low; spending has decreased as economic and social crises have deepened. | Rate: `8.0%` / FX Reserves: `$60B` . Policy is severely constrained by economic stagnation and capital flight risk. |
| **Russia** | `1.5%` (War-driven). Economy on a full war footing with massive state spending masking underlying issues. | `6.0%` CPI / `3.0%` Unemp. High inflation and labor shortages are direct results of the war economy. | `+2.5%` (Forced Surplus). Sanctions compress imports while high energy prices (to non-Western buyers) boost exports. | `20%` (Very low). A core policy for resilience. Sanctioned from Western debt markets, relies on domestic funding. | `~7.5%` (2024 est.). Extremely high share of GDP, reorienting the entire economy around military production. | Rate: `12%` / FX Reserves: `~$600B` . High sovereignty forced by sanctions; a test-case in de-dollarization. |
| **Indonesia** | `5.0%` (Robust growth). Stance is a mix of infrastructure spending and orthodox fiscal/monetary policy. | `3.0%` CPI / `5.5%` Unemp. Progress on poverty reduction, but formal job creation is a challenge. | `-0.5%` (Near balance). Commodity exports (palm oil, coal, nickel) are key to external stability. | `40%` (Low and stable). A key strength, providing policy space and resilience against capital flight. | `0.8%` (2024 data). Low military spending reflects a focus on domestic development and stability. | Rate: `6.0%` / FX Reserves: `$140B` . Prudent policy has earned market confidence, increasing monetary sovereignty. |
